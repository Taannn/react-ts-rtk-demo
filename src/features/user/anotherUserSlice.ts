...
export type KopiiUsers = {
  product_id: number;
  product_name: string;
}

type KopiiInitialState = {
  // loading: boolean;
  users: KopiiUsers[];
  error: string;
}

const initialState: KopiiInitialState = {
  // loading: false,
  users: [],
  error: ''
}
...
export const anotherFetchUsers = createAsyncThunk('users/anotherFetchUsers', async (_, { dispatch }) => {
  dispatch(setLoading(true));
  try {
    const response = await axios.get('http://localhost:3001/kopii/shop');
    return response.data.data;
  } catch (error) {
    throw error;
  } finally {
...
const anotherUserSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //   state.loading = true
    // })
    builder.addCase(anotherFetchUsers.fulfilled, (state, action: PayloadAction<KopiiUsers[]>) => {
      // state.loading = false
      state.users = action.payload
      state.error = ''
    })
    builder.addCase(anotherFetchUsers.rejected, (state, action) => {
      // state.loading = false
      state.users = []
